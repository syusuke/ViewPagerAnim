...
public class PageLeftInTransformer implements ViewPager.PageTransformer
{
    ...
    public void transformPage(View page, float position)
    {
        ...
        else if(position <= 1)
        {
            if(position < 0)
            {
                Log.e("positionass", "transformPage: " + position);
...
//                page.setTranslationX(width * (1 + position) * SCALE * -1.0f);
            }
        }
        else
        {
